...
class TeacherQualificationsPage extends StatefulWidget {
  ...
  @override
  State<TeacherQualificationsPage> createState() => _TeacherQualificationsPageState();
}
...
class _TeacherQualificationsPageState extends State<TeacherQualificationsPage> {
  ...
  Future<void> submit() async {
    ...
    if (docBytes != null) {
      docUrl = await uploadFile(docBytes!, 'teacher_docs/${widget.teacherUid}/qualification.pdf');
    }
    if (photoBytes != null) {
      photoUrl = await uploadFile(photoBytes!, 'teacher_docs/${widget.teacherUid}/photo.jpg');
    }
    ...
    final updateData = {
      ...
      'subjects': subjects
          .where((s) => s['name'].toString().trim().isNotEmpty && s['fees'].toString().trim().isNotEmpty)
          .toList(),
      'classes': classes
          .where((c) => c['name'].toString().trim().isNotEmpty && c['fees'].toString().trim().isNotEmpty)
          .toList(),
    };
          ...
          child: ListView(
            children: [
              TextFormField(
                ...
                validator: (v) => v == null || v.trim().isEmpty ? 'Enter your qualifications' : null,
              ),
              ...
              ...List.generate(classes.length, (i) {
                return Row(
                  ...
                  children: [
                    Expanded(
                      child: TextFormField(
                        key: ValueKey('class_name_${i}_${classes[i]['name']}'), // <-- Add key
                    ...
                    Expanded(
                      child: TextFormField(
                        key: ValueKey('class_fees_${i}_${classes[i]['fees']}'), // <-- Add key
              ...
              if (classes.length < 5)
                TextButton.icon(
                  ...
                  onPressed: () {
                    if (classes.length < 5) setState(() => classes.add({'name': '', 'fees': ''}));
                  },
                ),
              ...
              ...List.generate(subjects.length, (i) {
                return Row(
                  ...
                  children: [
                    Expanded(
                      child: TextFormField(
                        key: ValueKey('subject_name_${i}_${subjects[i]['name']}'), // <-- Add key
                    ...
                    Expanded(
                      child: TextFormField(
                        key: ValueKey('subject_fees_${i}_${subjects[i]['fees']}'), // <-- Add key
                        decoration: const InputDecoration(labelText: 'Fees'),
                        keyboardType: TextInputType.number,
              ...
              if (subjects.length < 5)
                TextButton.icon(
                  ...
                  onPressed: () {
                    if (subjects.length < 5) setState(() => subjects.add({'name': '', 'fees': ''}));
                  },
                ),
              const SizedBox(height: 24),
